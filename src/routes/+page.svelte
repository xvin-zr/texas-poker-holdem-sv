	...
	<section class="mx-auto flex max-w-5xl flex-col gap-6">
		...
		{#if game.status === 'idle'}
			<Card class="max-w-xl">
				...
				<CardContent class="space-y-4">
					<p class="text-muted-foreground">点击后进入第一手翻牌前阶段，所有存活玩家自动支付 Ante。</p>
		...
		{:else}
				...
				<CardContent class="flex flex-wrap gap-2">
					...
					{:else}
						{#each game.communityCards as card (card.rank + card.suit)}
							<span class="rounded-lg border bg-background px-3 py-2 font-mono" data-testid="community-card">
								{cardText(card)}
							</span>
						{/each}
					{/if}
			...
			<div class="flex gap-2">
				<Button disabled={game.currentActorId !== 'human'} onclick={() => act({ action: 'call' })}>跟注</Button>
				<Button variant="outline" disabled={game.currentActorId !== 'human'} onclick={() => act({ action: 'fold' })}>弃牌</Button>
					...
					{:else}
						<ol class="list-decimal space-y-1 pl-5">
							{#each game.actionHistory as action, index (index)}
								<li>{playerName(action.playerId)} 在 {stageText(action.stage)} 选择 {actionText(action.decision)}</li>
							{/each}
						</ol>
					{/if}
				</CardContent>
